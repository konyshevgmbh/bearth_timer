...
class _TimerPageState extends State<TimerPage> {
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 8),
                      _buildDetailRow(t.cycles, '${exercise.cycles}'),
                      _buildDetailRow(t.cycleDuration, t.secondsUnit(value: exercise.cycleDuration)),
                      _buildDetailRow(t.totalDuration, '${(exercise.cycles * exercise.cycleDuration / 60).toStringAsFixed(1)} min'),
                      _buildDetailRow(t.phases, '${exercise.phases.length}'),
                    ],
                  ),
                ),
              ],
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(_sessionService.currentExercise?.name ?? t.unknown),
      ...
      floatingActionButton: useHorizontalLayout ? null : FloatingActionButton.extended(
        ...
        label: Text(
          _sessionService.isRunning ? t.stop : t.start,
          style: TextStyle(
            color: Colors.black,
            fontSize: AppLayout.fontSizeMedium,
          ),
  ...
  Widget _buildScrollablePhasesList() {
    ...
    // Calculate cycle duration and full duration
    final cycleDuration = phases.fold(0, (sum, phase) => sum + phase.duration);
    final totalCycles = currentExercise.cycles;
    final fullDuration = cycleDuration * totalCycles;
    
    return ListView.separated(
      itemCount: phases.length + 1, // +1 for summary line
      ...
      itemBuilder: (context, index) {
          ...
          return Container(
            ...
            child: Row(
              ...
              children: [
                Expanded(
                  child: Text(
                    '$totalCycles × ${t.secondsUnit(value: cycleDuration)}  = ${t.secondsUnit(value: fullDuration)}',
                    style: TextStyle(
                      color: Theme.of(context).colorScheme.primary,
                      fontSize: AppLayout.fontSizeSmall,
                ...
                Text(
                  '${(fullDuration ~/ 60).toString().padLeft(2, '0')}:${(fullDuration % 60).toString().padLeft(2, '0')}',
                  style: TextStyle(
                    color: Theme.of(context).colorScheme.primary,
                    fontSize: AppLayout.fontSizeSmall,
                    fontWeight: FontWeight.w600,
        ...
        final clapsText = phase.claps == 1 ? t.oneClap : t.clapsCount(count: phase.claps);
        return ListTile(
          leading: Container(
            width: 20,
            height: 20,
          ...
          subtitle: hasRange ? Text(
                  '$clapsText • ${t.range(minDuration: phase.minDuration, maxDuration: phase.maxDuration)}',
                  style: TextStyle(
                    color: Theme.of(context).colorScheme.onSurfaceVariant,
          ...
          trailing: Text(
            t.secondsUnit(value: phase.duration),
            style: TextStyle(
              color: Theme.of(context).colorScheme.onSurface,
              fontSize: AppLayout.fontSizeSmall,
              fontWeight: FontWeight.w600,
      ...
      children: [
        ...
        Column(
          ...
          children: [
            ...
            Text(
              t.cycleProgress(current: _sessionService.currentCycle, total: _sessionService.currentExercise?.cycles ?? 0),
              style: TextStyle(
                color: Theme.of(context).colorScheme.onSurfaceVariant,
                fontSize: AppLayout.fontSizeSmall,
              ),
