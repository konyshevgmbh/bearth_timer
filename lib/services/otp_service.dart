...
class OTPService {
  ...
  Future<bool> sendOTP(String email, {bool isSignup = false}) async {
    if (!SupabaseConstants.isConfigured) {
      throw Exception(t.services.otp.supabaseNotConfigured);
    }
    ...
    try {
      ...
      throw Exception(t.services.otp.failedToSendOtp(error: e.message));
    } catch (e) {
      debugPrint('Send OTP error: $e');
      _updateStatus(OTPStatus.failed);
      throw Exception(t.services.otp.failedToSendOtpGeneric);
    }
  }
  ...
  Future<bool> verifyOTP(String otp) async {
    ...
    if (_currentEmail == null) {
      throw Exception(t.services.otp.noOtpRequest);
    }

    if (!hasValidOTP) {
      _updateStatus(OTPStatus.expired);
      throw Exception(t.services.otp.otpExpired);
    }
    ...
    try {
      ...
      if (e.message.contains('invalid') || e.message.contains('expired')) {
        throw Exception(t.services.otp.invalidOrExpiredOtp);
      }
      throw Exception(t.services.otp.otpVerificationFailed(error: e.message));
    } catch (e) {
      debugPrint('Verify OTP error: $e');
      _updateStatus(OTPStatus.failed);
      throw Exception(t.services.otp.otpVerificationFailedGeneric);
    }
  }
  ...
  Future<bool> updatePasswordAfterOTPVerification(String newPassword) async {
    ...
    if (_currentStatus != OTPStatus.verified) {
      throw Exception(t.services.otp.otpMustBeVerified);
    }
    ...
    try {
      ...
      throw Exception(t.services.otp.failedToUpdatePassword(error: e.message));
    } catch (e) {
      debugPrint('Update password error: $e');
      throw Exception(t.services.otp.failedToUpdatePasswordGeneric);
    }
  }
